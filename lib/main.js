 ...
 * Copyright (c) 2015 Adrian Price. All Rights Reserved.
 * @author Adrian Price <adrianp.quatinus@virginmedia.com>
 */
...
const PrefKeys = {
	...
	SAME_HOST_LENGTH_LIMIT : "sameHostLengthLimit",
	SAME_DOMAIN_LENGTH_LIMIT : "sameDomainLengthLimit",
...
const State = {
	...
	STOPPED : "stopped",
	RUNNING_LOADING : "running.loading",
	RUNNING_LOADED : "running.loaded",
	RUNNING_LOAD_TIMEOUT : "running.loadTimeout",
};
...
if (prefs[PrefKeys.RESUME_SESSION]) {
	var link = new Link(prefs[PrefKeys.URL]);
	if (!accept(link))
		link = randomSearch();
	addToHistory(link);
	session.state = State.RUNNING_LOADING;
	tabs.open({url : link.url, inBackground: false});
	if (prefs[PrefKeys.AUTO_CLICK])
	...
	switch (prefName) {
		case PrefKeys.AUTO_CLICK:
			...
			if (value)
				...
				createActionButton();
			break;
		case PrefKeys.USE_CURRENT:
			prefs[PrefKeys.URL] = tabs.activeTab.url;
			break;
...
function onTabLoad(tab) {
	...
	switch (session.state) {
		case State.RUNNING_LOADING:
			session.state = State.RUNNING_LOADED;
		case State.RUNNING_LOADED:
			...
			attachContentScript();
			break;
		case State.RUNNING_LOAD_TIMEOUT:
...
function startWalking() {
	...
	if (session.tab && session.tab.readyState == "complete") {
...
function stopWalking() {
	...
	clearLoadTimeout();
	clearDelayTimeout();
	if (session.state != State.RUNNING_LOAD_TIMEOUT) {
...
function onIdleTimeout() {
	...
	try {
		session.worker.port.emit("requestLinks")
	} catch (e) {
...
function followLink(link) {
	if (link && link.url != "") {
		...
		if (session.state == State.RUNNING_LOADED && session.worker && session.worker.port)
			session.worker.port.emit("nolinks");
		stopWalking();
	}
}

function setLoadTimeout() {
	var loadTimeout = prefs[PrefKeys.PAGE_LOAD_TIMEOUT];
	session.loadTimeoutID = loadTimeout > 0 ? setTimeout(onPageLoadTimeout, loadTimeout * 1000) : null;
}
...
function onPageLoadTimeout() {
	console.info("onPageLoadTimeout");
	...
	session.state = State.RUNNING_LOAD_TIMEOUT;
...
function currentLink() {
	return session.curHistoryIdx == -1 ? session.tab ? new Link(session.tab.url) : null : session.history[session.curHistoryIdx];
}

function selectRandomLink(urls) {
	console.info("selectRandomLink");

	// Filter the page's anchor hrefs to remove unsupported protocols and same-page links.
	var curLink = currentLink();
	var forwards = session.curHistoryIdx == session.history.length - 1;
	var links = [];
	var hostPartitionIdx = 0, domainPartitionIdx = 0;
	for (let i = 0; i < urls.length; i++) {
		var link = new Link(urls[i]);
		// Skip non-http[s] links and any which point to the same page (ignoring any query).
		if (link && link.urlNoQuery != curLink.urlNoQuery && validateLink(link)) {
			...
			// FIXME: ensure that we don't increment counts on a link to which we've back-tracked.
			if (forwards) {
				curLink.linkCount++;
				if (otherDomain)
	...
	console.info("  found " + urls.length + " links, " + links.length + " navigable, "
	...
	// Return the first acceptable link in the sorted, randomized list.
...
function validateLink(link) {
	return link.protocol == "http" || link.protocol == "https" || link.protocol == "file" && link.host != null;
}
...
function accept(link) {
	...
	try {
		...
		var lang = prefs[PrefKeys.LANGUAGE].trim();
		if (lang != "")
...
function randomSearch() {
	...
	return new Link(searchUrl);
}

// function findRandomWords() {
// 	var fileIO = require("sdk/io/file");
// // 	var wordsFile = "C:\\dev\\workspaces\\Firefox-addons\\random-walk\\WebContent\\data\\cg.al.o5";
// 	var wordsUrl = self.data.url("cg.al.o5");
// 	if (fileIO.exists(wordsFile)) {
// 		var TextReader = fileIO.open(wordsFile, "r");
